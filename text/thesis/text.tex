...
\midrule
PyTorch (FP32) & 98.9\% & 9,500 images \\
Python Simulation (INT8) & 97.6\% & 9,500 images \\
FPGA Hardware & 97.6\% & 9,500 images \\
\bottomrule
\end{tabular}
\end{table}
...
\textit{Note: Resource utilization values are reported from the Vivado post-synthesis summary.}
...
At 100 MHz: $\sim$5.8 ms per inference, or $\sim$172 images/second throughput.

\textit{Note: FC layer cycle counts include 2-cycle BRAM read latency wait states (MULT\_WAIT, MULT\_WAIT2) required for weight fetching from Block RAM.}
...
\section{Future Development of the Project}
